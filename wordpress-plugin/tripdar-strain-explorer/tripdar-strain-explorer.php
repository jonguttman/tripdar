...
class Tripdar_Strain_Explorer {
    ...
    public function handle_submit_report() {
        ...
        $slug = $data['strainSlug'];
        error_log('Tripdar: Submitting report to API for strain: ' . $slug);
        error_log('Tripdar: Report data - ' . print_r($data, true));
        $result = $this->api->submit_trip_report($slug, $data);
