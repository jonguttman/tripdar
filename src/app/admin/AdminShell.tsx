...
import type { ActiveViewAs, ViewAsUserOption } from "@/domain/auth/viewAs";
import {
  Alert,
  Button,
  Icon,
...
export default function AdminShell({
  ...
  activeViewAs: ActiveViewAs | null;
}) {
  ...
  async function startViewAs(userId: string) {
    if (!userId || viewAsLoading) return;
    ...
    if (!response?.ok) {
      ...
      setViewAsError(body?.error || "Could not start View as");
  ...
  async function exitViewAs() {
    if (viewAsLoading) return;
    setViewAsLoading(true);
    setViewAsError("");
    const response = await fetch("/api/view-as", { method: "DELETE" }).catch(() => null);
    if (!response?.ok) {
      setViewAsError("Could not exit View as");
      setViewAsLoading(false);
      return;
    }
    window.location.reload();
  ...
  const viewAsControl = (dark: boolean) => {
    ...
    return (
      <div className="space-y-1.5">
        <label
          htmlFor={dark ? "view-as-desktop" : "view-as-mobile"}
          className={cn(
            "block text-[0.625rem] font-semibold uppercase tracking-wider",
            dark ? "text-amber-300" : "text-amber-700"
          )}
        >
          View as partner admin
        </label>
        <select
          id={dark ? "view-as-desktop" : "view-as-mobile"}
          value={activeViewAs?.id ?? ""}
          disabled={viewAsLoading}
          onChange={(event) => startViewAs(event.target.value)}
          className={cn(
            "min-h-11 w-full rounded-lg border px-2 text-xs",
            dark
              ? "border-bark-600 bg-bark-800 text-bone-100"
        ...
        >
          ...
          {viewAsUsers.map((user) => (
            <option key={user.id} value={user.id}>
              {user.email} — {user.role} — {user.partnerName ?? "No partner"}
            </option>
          ))}
        </select>
        {viewAsError && <p className="text-xs text-clay-500">{viewAsError}</p>}
      </div>
    );
  };
    ...
    <div className="min-h-dvh bg-bone-100 font-sans text-bark-800">
      ...
      <main className="min-h-dvh pb-[calc(4.5rem+env(safe-area-inset-bottom))] md:pb-0 md:pl-64">
        {activeViewAs && (
          <div
            role="alert"
            className="sticky top-14 z-30 flex flex-col gap-2 border-b-2 border-amber-700 bg-amber-300 px-4 py-3 text-sm text-bark-900 shadow-md sm:flex-row sm:items-center sm:justify-between md:top-0"
          >
            <div>
              <strong>Viewing as {activeViewAs.email}</strong>
              <span className="ml-2">
                partner_admin · {activeViewAs.partnerName ?? "No partner assigned"} · read-only
              </span>
            </div>
            <button
              type="button"
              onClick={exitViewAs}
              disabled={viewAsLoading}
              className="min-h-10 shrink-0 cursor-pointer rounded-lg bg-bark-900 px-4 font-semibold text-bone-50 disabled:cursor-wait disabled:opacity-60"
            >
              {viewAsLoading ? "Exiting…" : "Exit View as"}
            </button>
          </div>
        )}
        {children}
