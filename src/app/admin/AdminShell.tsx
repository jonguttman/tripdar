...
import type { AdminIdentity, AdminViewAsUser } from "@/domain/auth/viewAs";
...
function viewAsUserLabel(user: Pick<AdminViewAsUser, "email" | "name" | "role" | "partnerName">) {
  const role = user.role === "super_admin" ? "super admin" : "partner admin";
  const partner = user.partnerName ?? "No partner";
  return `${user.email} - ${role} - ${partner}`;
}
...
export default function AdminShell({
  ...
  viewAs: AdminIdentity | null;
  viewAsUsers: AdminViewAsUser[];
}) {
  ...
  const canViewAs = viewAs?.actualRole === "super_admin";
  const isViewAsActive = Boolean(viewAs?.isViewAsActive);
  const exitViewAsHref = useMemo(
    () => `/api/admin/view-as/exit?returnTo=${encodeURIComponent(pathname || "/admin")}`,
    [pathname]
  );
  ...
  async function handleViewAsSubmit(e: React.FormEvent) {
    e.preventDefault();
    ...
    try {
      const res = await fetch("/api/admin/view-as", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ userId: viewAsUserId }),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        setViewAsError(data?.error?.message || "Could not enter View-as mode");
        setViewAsLoading(false);
        return;
      }
      window.location.reload();
    } catch {
      setViewAsError("Network error - try again");
      setViewAsLoading(false);
    }
  }
  ...
  const viewAsControl = (dark: boolean) => {
    ...
    if (isViewAsActive) {
      return (
        ...
        >
          <div className="text-[0.625rem] font-semibold uppercase tracking-wider">
            View-as active
          </div>
          <div className="mt-1 break-words text-sm font-semibold">
            {viewAs?.effectiveEmail}
          </div>
          <div className="mt-0.5 text-xs opacity-80">
            {viewAs?.effectiveRole === "super_admin" ? "Super admin" : "Partner admin"}
            {" - "}
            {viewAs?.effectivePartnerName ?? "No partner"}
          </div>
          <a
            href={exitViewAsHref}
            className={cn(
              "mt-3 flex min-h-10 w-full items-center justify-center rounded-lg text-xs font-semibold transition-colors",
              dark
                ? "bg-amber-200 text-bark-900 hover:bg-amber-100"
                : "bg-amber-700 text-bone-50 hover:bg-amber-800"
            )}
          >
            Exit View-as
          </a>
    ...
    return (
      <form onSubmit={handleViewAsSubmit} className="space-y-2">
        <label
          className={cn(
            "block text-[0.625rem] font-semibold uppercase tracking-wider",
            dark ? "text-bark-300" : "text-bark-400"
          )}
          htmlFor={dark ? "view-as-sidebar" : "view-as-drawer"}
        >
          View as
        </label>
        <select
          id={dark ? "view-as-sidebar" : "view-as-drawer"}
          value={viewAsUserId}
          onChange={(e) => setViewAsUserId(e.target.value)}
          className={cn(
            "min-h-11 w-full rounded-lg border px-3 text-sm outline-none transition-colors focus:border-moss-500 focus:ring-2 focus:ring-moss-500/20",
            dark
              ? "border-bark-700 bg-bark-800 text-bone-100"
              : "border-bone-300 bg-bone-50 text-bark-800"
          )}
        >
          <option value="">Choose user...</option>
          {viewAsUsers.map((user) => (
            <option key={user.id} value={user.id}>
              {viewAsUserLabel(user)}
            </option>
          ))}
        </select>
        {viewAsError && (
          <p className={cn("text-xs", dark ? "text-amber-200" : "text-clay-600")}>
            {viewAsError}
          </p>
        )}
        <Button
          ...
          disabled={!viewAsUserId || viewAsLoading}
          full
        >
          {viewAsLoading ? "Entering..." : "Enter View-as"}
        </Button>
      </form>
    );
  };
  ...
  const viewAsBanner = isViewAsActive ? (
    <div className="sticky top-0 z-30 border-b border-amber-400 bg-amber-300 px-4 py-3 text-bark-950 shadow-sm md:px-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <div className="mt-1 text-sm font-semibold">
            You are seeing admin as {viewAs?.effectiveEmail} (
            {viewAs?.effectiveRole === "super_admin" ? "super admin" : "partner admin"}
            {viewAs?.effectivePartnerName ? `, ${viewAs.effectivePartnerName}` : ", no partner"}).
          </div>
        </div>
        <a
          href={exitViewAsHref}
          className="inline-flex min-h-11 items-center justify-center rounded-lg bg-bark-950 px-4 text-sm font-semibold text-bone-50 transition-colors hover:bg-bark-800"
        >
          Exit View-as
        </a>
      </div>
