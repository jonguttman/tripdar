...
describe("staff review invite batches", () => {
  ...
  it("approval revokes all legacy pending invitations for the partner, not only draft employees", async () => {
    const { db, calls } = createDb({
      invitations: [
        pendingInvitation({ id: "prior_emp_a", employeeId: "emp_a" }),
        pendingInvitation({
          id: "prior_emp_z",
          employeeId: "emp_z",
        ...
        pendingInvitation({ id: "other_partner", partnerId: OTHER_PARTNER_ID, employeeId: "emp_other" }),
      ],
    });
    const prepared = await prepareDefaultBatch(db);
    calls.length = 0;
    ...
    expect(approved.revokedPriorInvitationCount).toBe(2);
    const findPriorPending = calls.find((call) => call.method === "staffReviewInvitation.findMany");
    const where = (findPriorPending?.args[0] as { where: Record<string, unknown> }).where;
    ...
    expect(db.staffReviewInvitation.updateMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { id: { in: ["prior_emp_a", "prior_emp_z"] }, status: "pending", revokedAt: null },
      })
    );
  });
