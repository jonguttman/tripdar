...
Core Function: Inform visitors about commonly reported experiential patterns associated with strain × dose context, focusing on how experiences tend to differ rather than on outcomes or benefits.
...
Microdose Note — Temporal Reference

In microdose contexts, meaningful variation is often temporal rather than dosage-based.

Tripdar accommodates this by allowing time-based reference frames within a fixed strain × dose context (e.g., earlier vs later in the day, cumulative exposure over time), without introducing metrics, accumulation, or intensity.

This does not:
	•	Add new dimensions
	•	Alter signal shape
	•	Introduce quantitative time measurement

It only refines which comparative references are meaningful when dose remains constant.
